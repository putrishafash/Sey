...
public class DetailPenyewaActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cursor = db.rawQuery("select * from penyewa, sepeda, sewa where penyewa.nama = sewa.nama AND sepeda.merk = sewa.merk AND penyewa.nama = '" + getIntent().getStringExtra("nama") + "'", null);
        cursor.moveToFirst();
        ...
        if (getSupportActionBar() !=null) {
            getSupportActionBar().setTitle("Detail PenyewaActivity");
        }
    }
